...
% - Custom Environments:
%   - ingredients [title=<alt title>, serves=N]
%   - timeline, directions, directions1col [<alt title>]
...
\usepackage{calc, etoolbox, nameref, xkeyval, xspace} % utilities
...
\ExplSyntaxOn
\keys_define:nn{ingredients} {
  title  .tl_set:N  = \ingrTitle,
  title  .initial:n = Ingredients,
  serves .tl_set:N  = \ingrServes,
}
\NewDocumentEnvironment{ingredients}{O{}} {
  \group_begin:
  \keys_set:nn{ingredients}{#1}
  \subsection{\ingrTitle \tl_if_empty:NF \ingrServes {~(serves~\ingrServes)}}
  \begin{ingredientslist}
  }
  {\end{ingredientslist}\group_end:}
\ExplSyntaxOff
...
\begin{directions}
  ...
  \item Heat 1 Tbsp oil until shimmering in large pot used for chorizo and add
    tomato puree. Add salt and sear over high heat for 10 minutes. Add the
    broth, oregano, black pepper, and diced tomatoes. Vigorously simmer for 10
    minutes.
...
\begin{twocols} \begin{ingredientslist}
  ...
  \item sweet potato casserole$^{\dagger \text{ (c.f.\ note)}}$ \todo[reference?]
  \item butternut squash (p\pageref{recipe:braised_butternut_squash})
...
\begin{twocols} \begin{kitchennotes}
  \item[Turkey] \begin{kitchensubnotes}
    ...
    \item In order to produce more drippings, piece the skin around the legs and
      thighs. Lift drumstick and cut 1 inch slit in skin where drumstick and
      thigh meet. Turn turkey on side so breast faces down and cut 1 inch slit
      in skin where top of thigh meets breast.
  ...
  \item[Miscellaneous]
    \begin{kitchensubnotes}
      ...
      \item Dressing and sweet potato casserole can be cooked at the same time.
    \end{kitchensubnotes}
...
\begin{IT}
  \begin{ingredients}
    ...
    \item[1.5 Tbsp] kosher salt
    \item[1.75 tsp] ground cardamom
    \item[1.75 tsp] brown sugar
    \item[1 tsp] ground ginger
    \item[1 tsp] ground nutmeg
    \item[1 tsp] smoked paprika
    \item[3.5 Tbsp] warm water
    \item[3.5 Tbsp] olive oil
  \end{ingredients}
...
\begin{twocols}
  \begin{ingredients}[title=Herbed Yogurt Ingredients]
...
\recipe{Street Food Style Chicken and Rice} \label{recipe:street_food_style_chicken_and_rice} % {{{1
\index{Street Food Style Chicken and Rice}
\index{chicken!Street Food Style Chicken and Rice}

\begin{IT}
  \begin{ingredients}[serves=4-6]
    \item[2 lbs] boneless skinless chicken thighs
    \item[1/2 cup ] plain yogurt
    ...
    \item[1 tsp] salt
    \item[1 tsp] pepper
    \item[1.5 cup] basmati rice
    \itemNL shredded iceberg lettuce
    \itemNL diced tomatoes
    \itemNL sliced red onion
  \end{ingredients}
...
\begin{directions}
  \item Cut the chicken into 1-inch pieces. Combine chicken, yogurt, lemon
  juice, coriander, cumin, garlic powder, turmeric, salt, and pepper in a large
  bowl and refrigerate for at least 1 hour or overnight.

  \item Prepare the white sauce (see below).

  \item Heat oven too 400\degF. Place chicken on wire rack in baking sheet and
  bake for about 20 minutes until it registers 160\degF. Finish the chicken off
  with the broiler for about 2 minutes.

  \item While the chicken is cooking cook the rice.

  \item Serve chicken with rice, lettuce, tomatoes, and onion. Dress with hot
...
\begin{twocols}
  \begin{ingredients}[title=White Sauce Ingredients]
    \item[1/2 cup] plain yogurt
  ...
  \begin{directions1col}[White Sauce Directions]
    \item Whisk together all ingredients and refrigerate for at least 1 hour or
    up to 3 days.
  \end{directions1col}
\end{twocols}
%---------------------------------------------------------------------------}}}1
...
\begin{kitchennotes}
  \item[Hot Dogs and Hamburgers] \label{recipe:hot_dogs_and_hamburgers}
    ...
    For 1 lb of meat, use 1/2 packet of Lipton's French Onion Soup mix and 1
...
\end{directions}
%---------------------------------------------------------------------------}}}1
\recipe{Braised Butternut Squash} \label{recipe:braised_butternut_squash} % {{{1
\index{Butternut Squash, Braised}
...
%---------------------------------------------------------------------------}}}1
\erecipe{Indian Black Eyed Peas (Lobhia)}{\InvIndianCooking[209]} % {{{1
\index{Indian Black Eyed Peas (Lobhia)|see{\InvIndianCooking[209]}}
\index{beans!Indian Black Eyed Peas (Lobhia)|see{\InvIndianCooking[209]}}
%---------------------------------------------------------------------------}}}1
\erecipe{Indian Carrots, Potatoes, Peas}{\InvIndianCooking[156]} % {{{1
\index{Indian Carrots, Potatoes, Peas|see{\InvIndianCooking[156]}}
\index{carrots!Indian Carrots, Potatoes, Peas|see{\InvIndianCooking[156]}}
%---------------------------------------------------------------------------}}}1
...
%---------------------------------------------------------------------------}}}1
\erecipe{Indian Mushrooms in Tomato Sauce}{\InvIndianCooking[163]} % {{{1
\index{Indian Mushrooms in Tomato Sauce|see{\InvIndianCooking[163]}}
\index{mushrooms!Indian Mushrooms in Tomato Sauce|see{\InvIndianCooking[163]}}
%---------------------------------------------------------------------------}}}1
...
\begin{IT}
  \begin{ingredients}
    ...
    \item[1/2 tsp] salt
    \item[1/2 cup] chopped walnuts or cacao nibs
...
\begin{directions}
  ...
  \item Whisk flour, baking powder, baking soda, and salt in medium bowl. Add to
  liquid ingredients and whisk until fully combined. Stir in the walnuts or
...
%---------------------------------------------------------------------------}}}1
\recipe{Chocolate Terrine} \label{recipe:chocolate_terrine} % {{{1
\index{Chocolate Terrine}

\begin{IT}[0.811]
  \begin{ingredients}
    \item[8 oz] bittersweet chocolate, chopped
    \item[8 oz] milk chocolate, chopped
    \item[10 Tbsp] butter, cut into 1-Tbsp pieces
    \item[1/2 tsp] salt
    \item[4] eggs, room temperature
    \item[1 Tbsp] sugar \todo[further reduce? original calls for 2]
...
\begin{directions}
  \item Heat oven to 325\degF and heat electric kettle. Microwave chocolate,
    butter, and salt in large bowl at 50\% power, stirring occasionally, until
    melted, 2-4 minutes. Let cool for 5 minutes.

  \item Whisk eggs and sugar until sugar is dissolved. While whisking, slowly
  add chocolate mixture and whisk until combined. Whisk in cream and milk until
  combined.

  \item Grease 8.5\texttimes 4.5 inch loaf pan with vegetable oil spray. Line
  pan with two strip of parchment paper, one 10\texttimes 8.5 inches and another
  that is 14\texttimes 4.5 inches, laying them orthogonal to one another.

  \item Pour chocolate mixture through strainer into pan and tap pan on counter
  a few times to release any bubbles. Tightly cover loaf pan with foil, place
  loaf pan in a baking pan, and pour boiling water from kettle into baking pan
  until it reaches halfway up the side of the loaf pan.

  \item Bake until outer inch of the terrine is set and the center is still
  jiggly, 1:00-1:10. Remove from oven, discard foil, and let cool in water bath
  for 1 hour. Wrap loaf pan with plastic wrap and refrigerate for at least 4
  hours.

  \item To serve, run knife around outer edge and release terrine by inverting
  pan on cutting board. Use a hot knife to cut the terrine, heating the blade
  after each slice.
\end{directions}
%---------------------------------------------------------------------------}}}1
\recipe{Coffee Cookies} \label{recipe:coffee_cookies} % {{{1
\index{Coffee Cookies}
\index{cookies!Coffee Cookies}

\begin{IT}
  \begin{ingredients}
    \item[2 cups (240 g)] all-purpose flour
    \item[1/2 tsp] baking soda
    \item[1/2 tsp] salt
    \item[2 Tbsp + 2 tsp] espresso powder
    \item[1 Tbsp] warm water
    \item[1/3 cup] sugar
    \item[10 Tbsp] butter, melted and cooled
    \item[2] eggs
...
\begin{directions}
  \item Heat oven to 350\degF and line 2 baking sheets with parchment paper.

  \item Whisk flour, baking soda and salt together in bowl. Whisk espresso
    powder and water until combined, then whisk sugar and melted butter until
    smooth. Add eggs and vanilla extract and whisk until combined. Fold in flour
    until dough forms, then fold in cacao nibs.

  \item Taking 2 Tbsp of dough at a time, roll into a ball, place onto prepared
    baking sheet, and flatten with bottom of a greased drinking glass to 1/2
    inch thickness.

  \item Bake cookies until edges are set but centers are still soft and puffy,
    about 13 minutes, rotating halfway through baking. Let cookies cool on sheet
...
\begin{IT}[0.753]
  \begin{ingredients}[title=Biga Ingredients]
  ...
  \end{ingredients}
  \begin{ingredients}[title=Bread Ingredients]
...
\end{directions}
%----------------------------------------------------------------------------}}}1
\recipe{G\^{a}teau Invisible} \label{recipe:gateau_invisible} % {{{1
\index{G\^{a}teau Invisible}
\index{cake!G\^{a}teau Invisible}
...
%---------------------------------------------------------------------------}}}1
\erecipe{Molasses Spice Cookies}{\CIBaking[203]} % {{{1
\index{Molasses Spice Cookies|see{\CIBaking[203]}}
\index{cookies!Molasses Spice Cookies|see{\CIBaking[203]}}
%---------------------------------------------------------------------------}}}1
...
\begin{IT}[0.695]
  \begin{ingredients}[title=Dutch Baby Ingredients]
  ...
  \begin{timeline}
    \item[Prep (dutch baby):]  00:20
    \item[Prep (topping):]  00:20
    \item[Cook:]  00:35
    \item[Total:] 00:55
  \end{timeline}
\end{IT}
...
\begin{twocols}
  \begin{ingredients}[title=Lox and Cucumber Ingredients]
  ...
  \begin{ingredients}[title=Mushroom and Red Pepper Ingredients]
...
\end{twocols}
%---------------------------------------------------------------------------}}}1
\erecipe{Strawberry Cream Cake}{\CIBaking[298]} % {{{1
\index{Strawberry Cream Cake|see{\CIBaking[298]}}
\index{cake!Strawberry Cream Cake|see{\CIBaking[298]}}
%---------------------------------------------------------------------------}}}1
...
\begin{twocols}
  \begin{ingredients}[title=Garlic Lemon Thyme]
    \index{sauces, pan!Garlic Lemon Thyme}
    \item[1] shallot, minced
    \item[2] cloves garlic
    \itemNL fresh thyme or 1 tsp dried
    \item[1 cup] chicken broth
    \item[1/2] lemon, juiced
    \item[1 Tbsp] butter
  \end{ingredients}

  \begin{ingredients}[title=Lemon Caper White Wine]
  ...
  \begin{ingredients}[title=Thyme Wine]
  ...
  \begin{ingredients}[title=Red Wine Rosemary]
  ...
  \begin{ingredients}[title=Red Wine Mushroom Thyme]
...
\begin{twocols}
  \begin{ingredients}[title=Garlic Rosemary Thyme Dijon]
  ...
  \begin{ingredients}[title=Roasted Garlic Parsley]
  ...
  \begin{ingredients}[title=Salt and Sugar]
...
\end{twocols}
%---------------------------------------------------------------------------}}}1
\recipe{Whipped Cream} \label{recipe:whipped_cream} % {{{1
\index{Whipped Cream}

\begin{ingredients}
  \item[1 cup] heavy cream
  \item[1 tsp] sugar
  \item[1.5 tsp] vanilla extract
\end{ingredients}

\begin{directions}
  \item Using stand mixer, whip all ingredients on medium-low speed until foamy,
    about 1 minute.

  \item Increase speed to high and whip until peaks form, about 1-3 minutes.
\end{directions}


\subsection{Variants}
\begin{twocols} \begin{kitchennotes}
  \item[Brown Sugar Whipped Cream] \begin{kitchensubnotes}
    \item Omit sugar and vanilla, add 1/2 cup sour cream, 2 Tbsp + 2 tsp
      light brown sugar \todo[original calls for 8 Tbsp --- this is 1/3 of
      that], and 1/8 tsp salt.
    \item Combine all ingredients and refrigerate for at least 4 hours, stirring
      once or twice so that the sugar dissolves. After sugar is dissolved,
      prepare as usual.
  \end{kitchensubnotes}

  \item[Brown Sugar Bourbon Whipped Cream] \begin{kitchensubnotes}
    \item Add 2 tsp bourbon before whipping.
  \end{kitchensubnotes}

  \item[Espresso Whipped Cream] \begin{kitchensubnotes}
    \item Omit sugar, add 1.5 tsp brown sugar \todo[should this be increased?
      ambiguous recipe] and 2 tsp espresso powder.
  \end{kitchensubnotes}

  \item[Tangy Whipped Cream] \begin{kitchensubnotes}
    \item Omit sugar, reduce vanilla to 1/8 tsp, add 1/4 cup sour cream and 2
      Tbsp light brown sugar \todo[original calls for 4 Tbsp, is this too
      sour?].
  \end{kitchensubnotes}

  \item[Stabilized Whipped Cream] \begin{kitchensubnotes}
    \item Sprinkle 1/2 tsp of gelatin over 1 Tbsp cold water and let sit for 5
      minutes to bloom it. Microwave in 5 second increments until mixture is
      liquid.
    \item Begin whipping and let gelatin mixture cool in the meantime. After
      soft peaks begin to form, temper the gelatin mixture by adding partially
      whipped cream, then sprinkle the mixture over the whipped cream with the
      mixer running on medium. Increase speed to high and whip until peaks form.
  \end{kitchensubnotes}
\end{kitchennotes} \end{twocols}
%---------------------------------------------------------------------------}}}1
